...
const GoogleLogo = () => (
  <svg width="20" height="20" viewBox="0 0 48 48" aria-hidden="true">
    <path
      fill="#EA4335"
      d="M24 9.5c3.54 0 6.72 1.22 9.22 3.6l6.9-6.9C35.9 2.6 30.47 0 24 0 14.62 0 6.4 5.38 2.56 13.22l8.95 6.95C13.25 14.02 18.18 9.5 24 9.5z"
    />
    <path
      fill="#4285F4"
      d="M46.5 24.5c0-1.64-.16-3.22-.46-4.76H24v9.02h12.7c-.55 2.96-2.24 5.46-4.78 7.16l7.3 5.66C43.86 37.8 46.5 31.75 46.5 24.5z"
    />
    <path
      fill="#FBBC05"
      d="M11.51 28.17A14.48 14.48 0 0 1 10.72 24c0-1.45.25-2.85.71-4.17l-8.95-6.95A23.94 23.94 0 0 0 0 24c0 3.87.93 7.53 2.56 10.78l8.95-6.61z"
    />
    <path
      fill="#34A853"
      d="M24 48c6.48 0 11.93-2.14 15.9-5.85l-7.3-5.66c-2.04 1.37-4.66 2.17-8.6 2.17-5.82 0-10.75-4.52-12.49-10.28l-8.95 6.61C6.4 42.62 14.62 48 24 48z"
    />
  </svg>
);
...
const Btn = styled.button<{ primary?: boolean }>`
  ...
  &:disabled {
    opacity: 0.5;
    cursor: not-allowed;
    background: ${(p) => (p.primary ? COLORS.brown : "#fafafa")};
    border-color: ${(p) => (p.primary ? COLORS.brown : COLORS.border)};
  }
`;
...
const RegisterPage: React.FC = () => {
  ...
  const canSubmit = useMemo(() => {
    ...
    return okName && okUser && okMail && okPass;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // ✅ Đổi key: fullname
      const payload = {
        fullname: fullName.trim(), // <-- đổi từ fullName -> fullname
      ...
      // Lưu lại info cần thiết để VerifyEmail/resend có thể dùng (tuỳ BE)
      // Lưu email để VerifyEmail hiển thị + verify
      localStorage.setItem("pendingEmail", payload.email);

      // Lưu full payload cho RESEND (password chỉ lưu tạm ở sessionStorage)
      const pendingReg = {
        fullname: payload.fullname,
        username: payload.username,
        email: payload.email,
        password: payload.password, // cần cho register-request nếu BE hash lại từ đây
      };
      sessionStorage.setItem("pendingReg", JSON.stringify(pendingReg));

      // (Tuỳ bạn) Nếu muốn phòng F5 toàn bộ và vẫn resend được:
      ...
      setNextPath("/auth/verify-email");
      setNavState({ email: payload.email });
      setSlidingOut(true);
    } catch (err: any) {
      const errorMsg =
            ...
            <Content appearing={appearing} slidingOut={slidingOut}>
              ...
              <Card>
                <form onSubmit={handleSubmit} noValidate>
                  ...
                  <Btn primary type="submit" disabled={!canSubmit}>
                    Sign up
                  </Btn>
                  <Sep>
                    <span>or</span>
